...
echo script name: $0
#TORCH_HOME="/home/wangxiaoxing"
TORCH_HOME="/mnt/nas/home/wangxiaoxing"
if [ "$TORCH_HOME" = "" ]; then
...
#for i in 2 3;do
  echo $j ${gpu[$j]}
  CUDA_VISIBLE_DEVICES=${gpu[$j]} OMP_NUM_THREADS=4 python3 -u ./exps/algos/PruneNAS.py \
	--save_dir ${save_dir} --max_nodes ${max_nodes} --channel ${channel} --num_cells ${num_cells} \
	--dataset ${dataset} --data_path ${data_path} \
	--search_space_name ${space} \
	--arch_nas_dataset ${benchmark_file} \
	--config_path configs/nas-benchmark/algos/ROME.config \
	--sal_type 'task' --num_compare 5 --reg_alpha 0.1\
    --track_running_stats ${BN} \
	--arch_learning_rate 0.0003 --arch_weight_decay 0.001 \
	--workers 4 --print_freq 200 --rand_seed ${seed} --task $i > $LOG_DIR/2-task$i.log  2>&1 &
  let j=($j+1)%$gpu_num
done
