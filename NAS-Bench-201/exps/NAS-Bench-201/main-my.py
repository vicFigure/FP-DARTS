...
def evaluate_all_datasets(arch, datasets, xpaths, splits, use_less, seed, arch_config, workers, logger, hyper_params=None):
    ...
    if hyper_params is None:
      ...
      config = load_config(config_path, \
                            {'class_num': class_num,
                             'xshape'   : xshape,
                             'LR': hyper_params[0],
#                             'momentum': hyper_params[1],
                             'decay': hyper_params[3]}, \
                            logger)
    ...
    all_valLoader_keys[dataset_key] = list(ValLoaders.keys())
    all_dataset_keys.append( dataset_key )
  all_infos['all_dataset_keys'] = all_dataset_keys
  all_infos['val_loader_keys'] = all_valLoader_keys
  return all_infos
...
def train_single_model(save_dir, workers, datasets, xpaths, splits, use_less, seeds, model_str, arch_config, hyper_params):
  assert torch.cuda.is_available(), 'CUDA is not available.'
  torch.backends.cudnn.enabled   = True
  torch.backends.cudnn.deterministic = True
  #torch.backends.cudnn.benchmark = True
  ...
  for _is, seed in enumerate(seeds):
    ...
    if to_save_name.exists():
      ...
      checkpoint = evaluate_all_datasets(arch, datasets, xpaths, splits, use_less, seed, arch_config, workers, logger, hyper_params)
      torch.save(checkpoint, to_save_name)
    # log information
    logger.log('{:}'.format(checkpoint['info']))
    all_dataset_keys = checkpoint['all_dataset_keys']
    valLoader_key = checkpoint['val_loader_keys']
...
#      logger.log('Last Info : Train = Acc@1 {:.2f}% Acc@5 {:.2f}% Error@1 {:.2f}%, Test = Acc@1 {:.2f}% Acc@5 {:.2f}% Error@1 {:.2f}%'.format(train_acc1es[last_epoch], train_acc5es[last_epoch], 100-train_acc1es[last_epoch], valid_acc1es[last_epoch], valid_acc5es[last_epoch], 100-valid_acc1es[last_epoch]))
      logger.log('Last Info : Train = Acc@1 {:.2f}% Acc@5 {:.2f}% Error@1 {:.2f}%'.format(train_acc1es[last_epoch], train_acc5es[last_epoch], 100-train_acc1es[last_epoch]))
      for key in valLoader_key[dataset_key]:
        val_idx = '{:}@{:}'.format(key,last_epoch)
        logger.log('%s = Acc@1 {:.2f}% Acc@5 {:.2f}% Error@1 {:.2f}%'.format(key, valid_acc1es[val_idx], valid_acc5es[val_idx], 100-valid_acc1es[val_idx]))
    # measure elapsed time
...
if __name__ == '__main__':
  ...
  parser.add_argument('--mode'   ,     type=str,   default='fromfile',  help='The script mode.')
  parser.add_argument('--base_dir',    type=str,                   help='Folder to the searched model.')
  ...
  parser.add_argument('--default_hp', action='store_true', default=False, help='use cutout')
  parser.add_argument('--gpu', type=int, default=0, help='gpu')
  args = parser.parse_args()

#  torch.cuda.set_device(args.gpu)

  assert args.mode in ['meta', 'new', 'cover', 'fromfile'] or args.mode.startswith('specific-'), 'invalid mode : {:}'.format(args.mode)
  ...
  elif args.mode == 'fromfile':
    search_ckpt = os.path.join(args.base_dir, 'search_final.pth')
    ckpt = torch.load(search_ckpt)
    genotype = ckpt['genotypes'][-1].tostr()
    if args.default_hp:
      hyper_params = None
    else:
      hyper_params = ckpt['hyper_params']
    train_single_model(args.save_dir, args.workers, args.datasets, args.xpaths, args.splits, args.use_less>0, \
                         tuple(args.seeds), genotype, {'channel': args.channel, 'num_cells': args.num_cells}, hyper_params)
