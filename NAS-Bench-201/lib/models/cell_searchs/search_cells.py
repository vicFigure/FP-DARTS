...
class NAS201SearchCell(nn.Module):
  ...
  def forward(self, inputs, weightss):
    ...
    for i in range(1, self.max_nodes):
      ...
      for j in range(i):
        ...
        inter_nodes.append( sum( layer(nodes[j]) * w if layer is not None else 0*w for layer, w in zip(self.edges[node_str], weights) ) )
  ...
  # ROME
  # It seems that forward_gdas is wrong (Line 59): if _ie != argmaxs, weigsum should add 0. but not weights[_ie]
  def forward_rome(self, inputs, hardwts, hardwts_edge, index):
    nodes   = [inputs]
    for i in range(1, self.max_nodes):
      inter_nodes = 0.
      for j in range(i):
        node_str = '{:}<-{:}'.format(i, j)
        if hardwts_edge[ self.edge2index[node_str] ] > 0:
          weights  = hardwts[ self.edge2index[node_str] ]
          argmaxs  = index[ self.edge2index[node_str] ].item()
#          weigsum  = sum( weights[_ie] * edge(nodes[j]) if _ie == argmaxs else 0. for _ie, edge in enumerate(self.edges[node_str]) )
          weigsum  = sum( weights[_ie] * edge(nodes[j]) if _ie == argmaxs else weights[_ie] for _ie, edge in enumerate(self.edges[node_str]) )
          inter_nodes += hardwts_edge[ self.edge2index[node_str] ] * weigsum
      nodes.append( inter_nodes )
    return nodes[-1]
