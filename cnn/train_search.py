...
parser.add_argument('--num_kept_max', type=int, default=8, help='The number of reserved ops after discretization or pruning')
parser.add_argument('--num_kept_min', type=int, default=8, help='The number of reserved ops after discretization or pruning')

parser.add_argument('--task', type=int, default=0, help='The number of reserved ops after discretization or pruning')
...
def main():
  ...
  if args.no_restrict:
    model.num_kept_min = args.num_kept_min
    model.num_kept_max = args.num_kept_max
  ...
  sal_queue = torch.utils.data.DataLoader(
      train_data, batch_size=32 if args.sal_second else args.batch_size,
      sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[split:num_train]),
      pin_memory=True, num_workers=0)
...
def infer(valid_queue, model, criterion, early_stop=-1, verbose=True):
  ...
  for step, (input, target) in enumerate(valid_queue):
    ...
    prec1, prec5 = utils.accuracy(outputs[-1], target, topk=(1, 5))
    n = input.size(0)
    objs.update(loss.item(), n)
    top1.update(prec1.item(), n)
    top5.update(prec5.item(), n)
...
def compute_sal(valid_queue, model, criterion, sal_type='task', sal_file=None, second_order=False):
  ...
  elif sal_type == 'sgas_1':
    alpha_normal, alpha_reduce = model.get_softmax_arch_parameters()
    mat = alpha_normal.detach()
    entropy = cate.Categorical(probs=mat).entropy() / math.log(mat.size()[1])
    score_normal = mat * utils.normalize(1 - entropy).unsqueeze(-1)
    mat = alpha_reduce.detach()
    entropy = cate.Categorical(probs=mat).entropy() / math.log(mat.size()[1])
    score_reduce = mat * utils.normalize(1 - entropy).unsqueeze(-1)
    n_saliencys = [score_normal, score_reduce]
    print("SGAS Saliency")
    print(n_saliencys)
    return n_saliencys
