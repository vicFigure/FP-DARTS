...
def select_device(device='', batch_size=None):
    ...
    s = f'FP-DARTS torch {torch.__version__} '  # string
...
#    genotype = eval("genotypes.%s" % args.arch)
    ...
    print('--------------------------') 
    if args.no_restrict:
      model = model_no_restrict.NetworkImageNet(args.init_channels, CLASSES, args.layers, args.auxiliary, genotype)
    else:
...
#    validdir = os.path.join(args.data, 'ILSVRC2012_img_val')
    ...
    nw = min([os.cpu_count() // args.world_size, args.workers])  # number of workers
    train_queue = torch.utils.data.DataLoader(
        train_data, batch_size=args.batch_size, shuffle=True, pin_memory=False, num_workers=nw)

    if rank in [-1, 0]:
        valid_queue = torch.utils.data.DataLoader(
            valid_data, batch_size=args.batch_size, shuffle=False, pin_memory=False, num_workers=nw)
...
#    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.decay_period, gamma=args.gamma, last_epoch=start_epoch)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if rank in [-1, 0]:
            ...
            ckpt = {
              'epoch': epoch,
              'state_dict': model.state_dict() if num_gpus<=1 else model.module.state_dict(),
