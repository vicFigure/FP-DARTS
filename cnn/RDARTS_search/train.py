...
def sample_genotype(prob_file=None):
  def sample_gene(gene_type, prob_op=None):
    gene = []
    n = 2
    start = 0
    step = 4
    ...
    if prob_op is None:
      if gene_type == 'normal':
        prob_op = [0, 0.1, 0.1, 0.16, 0.16, 0.16, 0.16, 0.16]
      elif gene_type == 'reduce':
        prob_op = [0, 0.2, 0.2, 0.12, 0.12, 0.12, 0.12, 0.12]
      else:
        raise(ValueError('no such gene_type: %s'%gene_type))
    ...
    for i in range(step):
      ...
      # sample edge
      if len(prob_op.shape)==1:
        edges = np.random.choice(range(i+2), min(numIn_per_node, i+2), replace=False)
      else:
        none_idx = genotypes.PRIMITIVES.index('none')
        W = prob_op[start:end].copy()
        actual_numIn = min(numIn_per_node, i+2)
        # remove the none Op
        max_idx = np.argpartition(-W, 2)
        p = np.zeros(W.shape[0])
        for j in range(W.shape[0]):
            if max_idx[j, 0] == none_idx:
                p[j] = W[j, max_idx[j,1]]
            else:
                p[j] = W[j, max_idx[j,0]]
        # normalize p
      ...
      # sample operator for each edge
      for j in edges:
        p = prob_op[j+start,:]
        # normalize p
        sum_p = np.sum(p)
        p = p / sum_p
        idx = np.random.choice(range(len(genotypes.PRIMITIVES)), 2, replace=False, p=p)
        if idx[0] != genotypes.PRIMITIVES.index('none'):
            idx = idx[0]
        else:
            idx = idx[1]
        gene.append((genotypes.PRIMITIVES[idx], int(j)))
      start = end
      n += 1
    return gene

  if prob_file is not None:
    print("Loading alpha probs from %s"%prob_file)
    probs = json.load(open(prob_file,'r'))
    normal_prob = probs['alphas_normal']
    reduce_prob = probs['alphas_reduce']
  else:
    normal_prob = None
    reduce_prob = None

  gene_normal = sample_gene(gene_type='normal', prob_op=normal_prob)
  gene_reduce = sample_gene(gene_type='reduce', prob_op=reduce_prob)

  step = 4; multiplier=4
  concat = list(range(2+step-multiplier, step+2))
...
def arch_eval_iter(choose_type='fromfile'):
  ...
  base_dir = 'search-EXP-SingleSepConv-20191014-101623'
  ...
  if choose_type == 'fromfile':
    ...
    eval_dir = os.path.join(base_dir, 'eval_sample_withoutNoneOp')
  ...
  for name in genotype_names:
    if choose_type == 'fromfile':
      ...
      print(genotype)
#      init_model = os.path.join(base_dir, 'eval_init_pt', 'eval_init20layer_%s.pt'%name)
#      model_index = '%s-searchInit20layer'%name
      init_model = None
      model_index = '%s-doubleSepConv-16C'%name
      best_acc, best_loss = train_from_scratch(args, train_queue, valid_queue, genotype, eval_dir, model_index, init_model)
      val_acc.append(best_acc)
      val_loss.append(best_loss)
      print('Best loss/acc: %f / %f'%(best_loss, best_acc))
...
#      alpha_file = None
  ...
  # save the results
  result_file = os.path.join(eval_dir, 'results-doubleSepConv-16C.csv')
...
def fulltrain_optim_arch(base_dir, genotype_names, epochs, choose_type='fromfile'):
  if not torch.cuda.is_available():
    logging.info('no gpu device available')
    sys.exit(1)
...
#  genotype_path = os.path.join(base_dir, 'results_of_7q/genotype_sal')
  ...
  print(choose_type)
  if choose_type == 'fromfile':
    eval_dir = os.path.join(base_dir, 'RDARTS/hyperband%d-%depochs'%(len(genotype_names), epochs))
  else:
...
#  base_dir = 'search-EXP-20200625-091629/'
  ...
  tmp_dir = os.path.join(base_dir, 'RDARTS')
  ...
  print("############\n")
  sorted_names = fulltrain_optim_arch(base_dir, genotype_names, epochs, choose_type='fromfile')
