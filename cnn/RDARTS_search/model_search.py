...
#          if torch.is_tensor(tmp): s += drop_path(tmp, drop_prob)
          if len(tmp.shape) == 4: s += drop_path(tmp, drop_prob)
      else:
...
#    return [self.softmax_alphas_normal, self.softmax_alphas_reduce]
  ...
  def genotype(self, no_restrict=False):
    ...
    def _parse(weights, prune_mask, normal, no_restrict=False):
      ...
      if no_restrict:
        ...
        idxes = np.sort(weights.argsort()[-8:])
        for idx in idxes:
          pos_r = int(idx / C)
...
#    self.update_softmax_arch_parameters()
    gene_normal, concat_normal = _parse(self.softmax_alphas_normal.data.cpu().numpy(), self.prune_masks[0].data.cpu().numpy(), True, no_restrict)
    gene_reduce, concat_reduce = _parse(self.softmax_alphas_reduce.data.cpu().numpy(), self.prune_masks[1].data.cpu().numpy(), False, no_restrict)
...
#        min_coords = [min_coords[idx] for idx in tmp_idx]
  ...
  def has_redundent_op(self, no_restrict=False):
    if no_restrict:
      if self.prune_masks[0].sum() > 8: return True
      if self.prune_masks[1].sum() > 8: return True
      return False
